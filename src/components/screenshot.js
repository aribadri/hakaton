...
import { getFaceRenderer, getFaceCamera, getFaceVideo, getFaceScene, renderFaceFrame } from "../faceScene.js";
  ...
  init: function () {
    ...
    captureBtn.addEventListener("click", () => {
      ...
      if (mode === "face") {
        ...
        const maskContainer = document.querySelector(".mask-content");
        const allCanvases = maskContainer?.querySelectorAll("canvas");
        const video = getFaceVideo();

        if (!video || !allCanvases || allCanvases.length === 0) return;
        ...
        allCanvases.forEach(faceCanvas => {
          const fcw = faceCanvas.width;
          const fch = faceCanvas.height;

          if (fcw === 0 || fch === 0) return;

          const faceAspect = fcw / fch;
          const canvasAspect = cw / ch;

          let fsx, fsy, fsw, fsh;
          if (faceAspect > canvasAspect) {
            fsh = fch;
            fsw = fch * canvasAspect;
            fsx = (fcw - fsw) / 2;
            fsy = 0;
          } else {
            fsw = fcw;
            fsh = fcw / canvasAspect;
            fsx = 0;
            fsy = (fch - fsh) / 2;
          }

          ctx.drawImage(faceCanvas, fsx, fsy, fsw, fsh, 0, 0, cw, ch);
        });
