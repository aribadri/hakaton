...
class C {
  constructor({
    ...
    (this.container = a),
      ...
      (this.disableFaceMirror = l),
      (this.scene = new m()),
      (this.cssScene = new m()),
      (this.renderer = new g({ antialias: !0, alpha: !0 })),
      (this.cssRenderer = new E({ antialias: !0 })),
      (this.renderer.outputEncoding = y),
      this.renderer.setPixelRatio(window.devicePixelRatio),
      (this.camera = new w()),
      (this.userDeviceId = s),
      (this.environmentDeviceId = c),
      (this.anchors = []),
      (this.faceMeshes = []),
      (this.latestEstimate = null),
      this.container.appendChild(this.renderer.domElement),
      this.container.appendChild(this.cssRenderer.domElement),
      (this.shouldFaceUser = !0),
      window.addEventListener("resize", this._resize.bind(this));
  }
  async start() {
    // this.ui.showLoading(),
    await this._startVideo(), await this._startAR();
    // this.ui.hideLoading();
  }
  stop() {
  ...
  addFaceMesh() {
    const a = this.controller.createThreeFaceGeometry(I),
      t = new M(a, new b({ color: 16777215 }));
    return (
      (t.visible = !1), (t.matrixAutoUpdate = !1), this.faceMeshes.push(t), t
    );
  }
  addAnchor(a) {
    const t = new p();
  ...
  _startVideo() {
    return new Promise((a, t) => {
      ...
      this.shouldFaceUser
        ? this.userDeviceId
          ? (o.video.deviceId = { exact: this.userDeviceId })
          : (o.video.facingMode = "environment")
        : this.environmentDeviceId
        ? (o.video.deviceId = { exact: this.environmentDeviceId })
        : (o.video.facingMode = "environment"),
  ...
  _startAR() {
    return new Promise(async (a, t) => {
      const o = this.video;
      this.container,
        (this.controller.onUpdate = ({ hasFace: e, estimateResult: h }) => {
          for (let s = 0; s < this.anchors.length; s++)
            this.anchors[s].css
              ? this.anchors[s].group.children.forEach((c) => {
          ...
          if (e) {
            ...
            for (let r = 0; r < this.anchors.length; r++) {
              const v = this.anchors[r].landmarkIndex,
                i = this.controller.getLandmarkMatrix(v);
              ...
              } else this.anchors[r].group.matrix.set(...i);
            }
            for (let r = 0; r < this.faceMeshes.length; r++)
              this.faceMeshes[r].matrix.set(...c);
          } else this.latestEstimate = null;
        }),
        this._resize();
      const n = this.shouldFaceUser && !this.disableFaceMirror;
      await this.controller.setup(n),
        await this.controller.dummyRun(o),
        this._resize(),
        this.controller.processVideo(o),
        a();
    });
  }
  _resize() {
    ...
    if (!e) return;
    {
      this.video.setAttribute("width", this.video.videoWidth),
        this.video.setAttribute("height", this.video.videoHeight),
        this.controller.onInputResized(e);
      const {
        fov: v,
        aspect: i,
        near: f,
    ...
    let h, s;
    const c = e.videoWidth / e.videoHeight,
      l = n.clientWidth / n.clientHeight;
    c > l
      ? ((s = n.clientHeight), (h = s * c))
      : ((h = n.clientWidth), (s = h / c)),
      (e.style.top = -(s - n.clientHeight) / 2 + "px"),
      (e.style.left = -(h - n.clientWidth) / 2 + "px"),
      (e.style.width = h + "px"),
      (e.style.height = s + "px"),
      this.shouldFaceUser && !this.disableFaceMirror
        ? (e.style.transform = "scaleX(-1)")
        : (e.style.transform = "scaleX(1)");
    const d = a.domElement,
      r = t.domElement;
